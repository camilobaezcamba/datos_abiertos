...
define(['trademapper.csv.definition', 'trademapper.route', 'util', 'd3'], function(csvdefs, route, util, d3) {
	...
	setFileInputElement: function(fileInput) {
		...
		if (this.fileInputElement !== null) {
			var moduleThis = this;
			this.fileInputElement.on('click', function() {moduleThis.loadCSVFile();});
		}
	},
	
	setFileInputElement2: function(fileInput2) {
		this.fileInputElement2 = fileInput2;
		if (this.fileInputElement2 !== null) {
			var moduleThis = this;
			this.fileInputElement2.on('click', function() {moduleThis.loadCSVFile2();});
		}
	},
	...
	loadCSVFile: function() {
		this.resetLoadErrors();
		var moduleThis = this;
		var anho = jQuery("[id$='anho']").val();
		jQuery("[id$='ayuda']").show();
		$.ajax({
		    url: 'http://localhost:8080/opendata-ws-restful/importacionesPorAnho/'+anho,
		    type: 'GET',
		    data: '', 
		    success:function(data) {
		    	var csvFile = JSONToCSVConvertorMapa(data.objeto,"import");
		    	var reader = new FileReader();
					moduleThis.loadingCsv = true;
					moduleThis.processCSVString(csvFile);
					moduleThis.loadingCsv = false;
					jQuery("[id$='ayuda']").hide();
		    },
		    error:function() {
					jQuery("[id$='ayuda']").hide();
		    }
	...
	loadCSVFile2: function() {
		this.resetLoadErrors();
		var moduleThis = this;
		var anho = jQuery("[id$='anho']").val();
		jQuery("[id$='ayuda']").show();
		$.ajax({
		    url: 'http://localhost:8080/opendata-ws-restful/exportacionesPorAnho/'+anho,
		    type: 'GET',
		    data: '',
		    success:function(data) {
		    	var csvFile = JSONToCSVConvertorMapa(data.objeto,"import");
