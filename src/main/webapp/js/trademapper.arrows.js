...
define(["d3", "spiralTree", "trademapper.route", "util"], function(d3, flowmap, tmroute, util) {
	...
	drawRoute: function(route) {
		...
		this.arrowg.append("path")
			...
			.on('click', this.createPlainMouseOverFunc(route))
	...
	plainMouseOverPath: function(route) {
		...
		tooltiptext += '<span class="tooltip-total">Total:</span><br />';
		tooltiptext += '<br /><span class="tooltip-units">Moneda: USD</span>';
	...
	drawLegend: function() {
		...
		var gLegend, i, strokeWidth, value, valueText, circleX, circleY,
			...
			legendHeight = Math.max(140, margin*4 + 8 + roundUpWidth(1) + roundUpWidth(0.5) + roundUpWidth(0.25)),
		...
		for (i = 0; i < 4; i++) {
			...
			valueText = formatoMiles(value.toFixed(0));
			if (i === 3) {
				valueText = "< " + valueText;
			}
